...
function Single({ row, apiUrl }) {
  ...
  const deleteProject = async () => {
    try {
      dispatch(setDelete());
      const response = await fetch(`${apiUrl}/api/project/${row.id}`, {
        method: "DELETE",
        headers: {
      ...
      <div className="box custom-box">
            ...
            {isOpen && (
              ...
              >
                ...
                <li>
                  <Link
                    className="ti-dropdown-item"
                    href={`/project-management/projects/edit/${row.id}`}
                    scroll={false}
                  >
        ...
        <div className="box-footer flex items-center justify-between">
          <div>
            ...
            <span className="font-semibold block">{row.start_date}</span>
          </div>
          <div className="text-end">
            <span className="text-[#8c9097] dark:text-white/50 text-[0.6875rem] block">
              Due Date :
            </span>
            <span className="font-semibold block">{row.end_date}</span>
          </div>
        </div>
      </div>
    </div>
