...
const EditProgramStatus = ({ params }) => {
  const { id } = params;
  ...
  const getStatus = async () => {
    const res = await fetch(`${apiUrl}/api/programs-status/${id}/edit`, {
      method: "GET",
      headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const res = await fetch(`${apiUrl}/api/programs-status/${id}`, {
      method: "PUT",
      headers: {
  ...
  return (
    <Fragment>
      <Seo title={"Edit Program Status"} />
      <Pageheader
        currentpage="Edit Program Status"
        activepage="Program Statuses"
        activeurl="/referential/program-status"
        mainpage="Edit Program Status"
      />
