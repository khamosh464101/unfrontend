...
const StaffStatus = () => {
  ...
  const [url, setUrl] = useState(`${apiUrl}/api/staffs-statuses`);
  ...
  const deleteStatus = async (id) => {
    try {
      dispatch(setDelete());
      const response = await fetch(`${apiUrl}/api/staffs-status/${id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${session.access_token}`,
  ...
  return (
    <div>
      <Seo title={"Staff Statuses"} />
      <Pageheader
        currentpage="Staff Statuses"
        activepage="Tables"
        mainpage="Staff Statuses"
      />
          ...
          <div className="box">
            <div className="box-header">
              <div className="flex justify-between gap-4">
                <Link
                  href="/referential/staff-status/create"
 ...
 table-hover min-w-full ti-custom-table-hover"
  ...
  align-middle me-1 `}
                            ...
                            <td className="flex gap-2">
                              ...
                              <Link
                                href={`/referential/staff-status/edit/${row.id}`}
