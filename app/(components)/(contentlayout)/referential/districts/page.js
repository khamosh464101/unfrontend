...
const Districts = () => {
  ...
  const [url, setUrl] = useState(`${baseUrl}/api/districts`);
  ...
  }, [url, session, sortBy, deleteItem, provinceId]);
  ...
  const deleteProvince = async (id) => {
    try {
      dispatch(setDelete());
      const response = await fetch(`${baseUrl}/api/district/${id}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${session.access_token}`,
 ...
 table-hover min-w-full ti-custom-table-hover"
                        ...
                        districts.data.map((row, index) => (
                          <tr className="border-b border-defaultborder">
                            ...
                            <td className="flex gap-2">
                              ...
                              <Link
                                href={`/referential/districts/edit/${row.id}`}
                  ...
                  <nav aria-label="Page navigation">
                    <ul className="ti-pagination ltr:float-right rtl:float-left mb-4">
                      {districts.links &&
                        districts.links.length > 3 &&
                        districts.links.map((row, index) => (
                          <li
                            className={`page-item ${
